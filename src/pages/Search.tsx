...
const Search = () => {
  ...
  const seoMetadata = useMemo(() => {
    ...
    let title = "ピアノ教室・リトミック教室を探す";
    let description = "全国のピアノ教室・リトミック教室を検索できます。";
    let keywords = "ピアノ教室,リトミック教室,音楽教室,検索";

    // エリア指定がある場合
    if (areaParam || selectedPrefecture) {
      const area = areaParam || selectedPrefecture;
      title = `${area}のピアノ教室・リトミック教室`;
      description = `${area}でピアノ教室・リトミック教室をお探しですか？${area}の優良な音楽教室を検索できます。`;
      keywords += `,${area}`;
    }

    // レッスンタイプ指定がある場合
    if (typeParam === 'piano' || lessonParam === 'piano' || selectedLessonTypes.includes('piano')) {
      title = selectedPrefecture ? `${selectedPrefecture}のピアノ教室` : "ピアノ教室を探す";
      description = "ピアノレッスンを受けられる教室を検索できます。初心者から上級者まで、あなたに最適なピアノ教室を見つけてください。";
      keywords += ",ピアノレッスン,ピアノ";
    }

    if (typeParam === 'eurythmics' || lessonParam === 'eurythmics' || selectedLessonTypes.includes('eurythmics')) {
      title = selectedPrefecture ? `${selectedPrefecture}のリトミック教室` : "リトミック教室を探す";
      description = "リトミックレッスンを受けられる教室を検索できます。お子様の音楽的感性を育てるリトミック教室を見つけてください。";
      keywords += ",リトミックレッスン,リトミック,子供";
    }

    // キーワード検索がある場合
    if (keywordParam || keyword) {
      const searchKeyword = keywordParam || keyword;
      title = `「${searchKeyword}」の検索結果`;
      description = `「${searchKeyword}」に関連するピアノ教室・リトミック教室の検索結果です。`;
      keywords += `,${searchKeyword}`;
    }

    return { title, description, keywords };
  }, [searchParams, selectedPrefecture, selectedLessonTypes, keyword]);
